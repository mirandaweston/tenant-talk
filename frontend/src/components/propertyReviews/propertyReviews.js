    ...
    return (
      <div className="overflow-hidden bg-white shadow sm:rounded-lg">
        <div className="px-4 py-5 sm:px-6">
          <h3 className="text-base font-semibold leading-6 text-gray-900">{data.property.address}</h3> 
        
          <p className="mt-1 text-sm text-gray-500">{"Reviews: " + data.property.reviews.length}</p>
        </div>
          ...
          <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
            <div className="sm:col-span-1">
              ...
              {[0, 1, 2, 3, 4].map((rating) => (
                ...
                <p className="mt-1 text-sm text-gray-500">{getAverage(data.property)} out of 5 stars</p>
              </div>
              </dd>
            </div>
            {/* <div className="sm:col-span-1">
